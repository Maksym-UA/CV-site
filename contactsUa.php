...
$errEmail = "Будь ласка, введіть адрес електронної пошти.";
} else {
    $email = test_input($_POST["email"]);
    // check if e-mail address is valid format
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      $errEmail = "Невірний формат адреси.";
    }
}
...
$errMessage = "Будь ласка, залиште своє повідомлення.";
} else {
    $message = test_input($_POST["message"]);
}
//Check if simple anti-bot test is entered
if (empty($_POST["human"])) {
$errHuman = "Будь ласка, введіть суму.";
} else {
     if ($human !== 24) {
     $errHuman = 'Невірна відповідь. Будь ласка, спробуйте ще.';
        }
}
// If there are no errors, send the email & output results to the form
if (!$errName && !$errEmail && !$errPhone &&  !$errMessage && !$errHuman) {
    if (mail ($to, $subject, $body, $from)) {
        $result='<div class="alert alert-success"><h2><span class="glyphicon glyphicon-ok"></span> Повідомлення відправлено!</h2><h3>Дякуємо, що звернулись до нас. Незабаром ми зв’яжемось з Вами.</h3></div>';
    } else {
        $result='<div class="alert alert-danger"><h2><span class="glyphicon glyphicon-warning-sign"></span> Вибачте, сталася помилка.</h2> <h3>Будь ласка, спробуйте ще раз пізніше.</h3></div>';
       }
    }
}
...
<html lang="en">
	...
	<header>
			...
			<ul class="topnav" id="myTopnav">
			  ...
			  <li><a href="indexUA.html">Резюме</a></li>
			  <li><a href="portfolio_Ua.html">Порфоліо</a></li>
			  <li><a class="active" href="contactsUa.php">Контакт</a></li>
		...
		<section class="main">
			...
			<div class="container-fluid text-center">
				...
				<h2> зв’язатись...</h2>
					...
					<h3> або залишити повідомлення</h3>	
					<p class="required small">* = Заповнити обов’язково</p>
					<form class="form-horizontal" role="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
						...
						<div class="form-group">
						<label for="name" class="col-sm-3 control-label"><span class="required">*</span> Ім’я:</label>
						<div class="col-sm-7">
						<input type="text" class="form-control" id="name" name="name" placeholder="Ім’я та прізвище" value="<?php echo $name;?>">
						...
						<div class="form-group">
						<label for="message" class="col-sm-3 control-label"><span class="required">*</span> Повідомлення:</label>
						<div class="col-sm-7">
						<textarea class="form-control" row="4" name="message" placeholder="повідомлення"><?php echo $message;?></textarea>
						<span class="required small"><?php echo $errMessage;?></span>
						</div>
						</div>

						<div class="form-group">
						<label for="human" class="col-sm-3 control-label"><span class="required">*</span> Тест:</label>
						<div class="col-sm-4">
						<h3 class="human">(6 + 6) * 2 = ?</h3>
						<input type="text" class="form-control" id="human" name="human" placeholder="Ваша відповідь" value="<?php echo $human;?>">
						...
						<button type="submit" id="submit" name="submit" class="btn-lg btn-primary btn-block">НАДІСЛАТИ</button>
						</div>
						</div>
					</form>
